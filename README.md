...
- [Второй интерфейс](https://github.com/sanmai/pindx-client) подразумевает получение данных отделения по [JSON API почтовых индексов](https://www.postindexapi.ru/). Это можно сделать только через [запрос к API](https://www.postindexapi.ru/), потому этот метод работает более медленно, чем первый. С другой стороны, использование этого метода исключает необходимость время от времени обновлять версию пакета для обновления базы индексов.
